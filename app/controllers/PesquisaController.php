...
class PesquisaController extends Controller{
  ...
  //Pesquisa para tabela de processo    
    public function ConsultaProcesso(){
        ...
        if(isset($_POST['tabela']) && !empty($_POST['valorPreenchidoUsuario'])){
...
public function pegarDadosDoUsuario(){
     if(isset($_POST['pesquisa']) && !empty('pesquisa')){
         ...
         $valorPreenchidoUsuario = $_POST['valorPreenchidoUsuario'];
               switch($pesquisa){
                ...
                $tabela = isset($_SESSION['tabela']) ? $_SESSION['tabela'] : $tabelaPesquisa;
     ...
     if(isset($_POST['valorPreenchidoUsuario']) && !empty($_POST['valorPreenchidoUsuario'])){
          ...
          if($campo == "nome_servidor" OR $campo == "cpf"){
             $dados['processo'] = $consulta->porServidor($condicao);
              foreach($dados['processo'] as $np){
                $numero_processo = $np->numero_processo;    
                $dados['processado'] = $this->PegarProcessadoPorProcesso($numero_processo);
              }
          }
    ...
    public function PegarProcessadoPorProcesso($numero_processo){
            ...
            $dados["processado"] = $processo->PegarProcessado($numero_processo);
            if(($dados["processado"])){
                return $dados["processado"];
            }else{
                return "O processo número ".$numero_processo ." não tem processado";
            }
    }
...
public function contarRegistro(){
     if(isset($_POST['valorPreenchidoUsuario']) || $_GET['p']){
          $tabela = isset($_SESSION['tabela']) ? $_SESSION['tabela'] : "processo";
          $campo = $_SESSION['campo'];
     ...
     public function antif5(){
        if(isset($_GET['valorPreenchidoUsuario'])){
          $valor = $_GET['valorPreenchidoUsuario'];
